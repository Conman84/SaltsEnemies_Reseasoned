...
namespace SaltsEnemies_Reseasoned
{
    public static class RBYPFlowers
    {
        public static void Add()
        {
            ...
            splatter.name = "Splatter_4_PA";
            splatter.AddToPassiveDatabase();
            ...
            {
                Description = "If the Opposing party member has Power, deal an Agonizing amount of damage to them. Apply 5 Power on the Opposing party member.\nApply 1 Photosynthesis to this enemy.",
                Rarity = Rarity.GetCustomRarity("rarity5"),
                Effects = new EffectInfo[]
                {
                    Effects.GenerateEffect(ScriptableObject.CreateInstance<KillIfPowerEffect>(), 10, Slots.Front),
            ...
            loveu.AddIntentsToTarget(Targeting.Slot_Front, new string[] { IntentType_GameIDs.Damage_7_10.ToString(), Power.Intent });
            ...
            {
                Description = "Heal the Opposing party member a Little health. Deal an Agonizing amount of damage to the Opposing party member if they were not healed. \nApply 1 Photosynthesis to this enemy.",
                Rarity = Rarity.GetCustomRarity("rarity5"),
                Effects = new EffectInfo[]
                {
                    Effects.GenerateEffect(ScriptableObject.CreateInstance<HealEffect>(), 3, Targeting.Slot_Front),
                    Effects.GenerateEffect(ScriptableObject.CreateInstance<DamageEffect>(), 10, Slots.Front, BasicEffects.DidThat(false)),
                    Effects.GenerateEffect(ScriptableObject.CreateInstance<ApplyPhotoSynthesisEffect>(), 1, Targeting.Slot_SelfSlot)
                },
                Visuals = CustomVisuals.GetVisuals("Salt/Rain"),
                AnimationTarget = Slots.Front
            };
            cryu.AddIntentsToTarget(Targeting.Slot_Front, new string[] { IntentType_GameIDs.Heal_1_4.ToString(), IntentType_GameIDs.Damage_7_10.ToString() });
